...
async function publishDialogueUpdate(options, jobId) {
  ...
  printJsonOrLines(options, result, [
    ...
    ...(kind === "question"
      ? [`The answer arrives as an approved follow-up relay: keep running lumine sponsor job pulse ${jobId} and read the refreshed assignment.`]
      : []),
  ]);
}
...
2. Introduce yourself and publish meaningful milestones with \`lumine sponsor job update ${jobState.job.id} --file <message-file> --phase <name>\`. Speak as Lumine, never as the underlying provider. To ask the user something you genuinely cannot decide yourself, add \`--kind question\`: ${displayPersona(jobState.job.persona)} puts it to the user and their answer comes back as an approved follow-up relay on your next pulse. Keep working on what does not depend on the answer.
...
${consultation ? "6" : "7"}. Finish with \`lumine sponsor job complete ${jobState.job.id} --summary "..."\`. ${consultation ? `For this consultation, the summary is the exact substantive answer ${displayPersona(jobState.job.persona)} will share; no artifact or project change is created.` : "The CLI will save a canonical artifact directly to the approved workspace. Publishing remains a separate owner action."}
`;
...
async function withSponsorStateLock(options, operation) {
  ...
  let handle;
  // Short check-ins (the lease keeper, a pulse) hold the lock for well under
  // a second, so a colliding command waits briefly instead of failing.
  const waitUntil = Date.now() + SPONSOR_LOCK_WAIT_MS;
  for (;;) {
    try {
