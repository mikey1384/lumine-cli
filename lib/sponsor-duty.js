...
const EXECUTION_MODE = "agent_session_v1";
...
async function reconcileExistingStateBeforeStart({
  ...
  if (preservedWorkspaces.length > 0) {
    const archivePath = await archiveSponsorState(options);
    console.error(
...
async function changeDutyState(options, action) {
    ...
    if (action === "stop") {
      ...
      const localArchive =
        preservedWorkspaces.length > 0
          ? await archiveSponsorState(options)
          : null;
  ...
  if (localState && Number(localState.duty?.id) === dutyId) {
    if (nextState === "stopped") {
      ...
      if (preservedWorkspaces.length > 0) {
        localArchive = await archiveSponsorState(options);
      } else {
        await removeSponsorState(options);
      }
    } else {
...
async function completeJob(options, jobId, commandServices) {
  ...
  if (!summary) {
    throw new Error("Summarize the actual completed work with --summary <text>.");
  }
  const auth = await ensureSponsorAuth(options);
  let state = await loadOwnedState({ options, auth });
  ({ state } = await heartbeatJob({ options, auth, state, jobId }));
  ...
  printJsonOrLines(options, result, [
    `Completed Workshop job #${jobId}; ${displayPersona(jobState.job.persona)} sent the canonical branch update.`,
  ]);
}
...
async function heartbeatJob({ options, auth, state, jobId }) {
  ...
  if (newRelayCount > 0 && jobState.preparedAt) {
    await writeAssignment(jobState, state);
  }
  await writeSponsorState(options, state);
  return { state, newRelayCount };
...
async function prepareClaimedJob({
  ...
  if (jobState.workspaceToken?.accessToken) {
    await writeAuthFile(jobWorkspaceOptions(options, state, jobState), {
      ...
      username:
        ...
        displayPersona(jobState.job.persona),
      userId:
        jobState.workspaceToken.user?.id || Number(jobState.job.personaUserId),
  ...
  await commandServices.pullWorkspace({
    options: jobOptions,
    auth: jobAuth,
    buildId: Number(jobState.job.contributionBuild.id),
  });
  jobState.forumContext = await loadForumContext({
    options: jobOptions,
    auth: jobAuth,
    buildId: Number(jobState.job.contributionBuild.id),
  });
...
async function ensureCanonicalBranchNotice({
  ...
  try {
    const notice = await notifyBuildOwnerOfContribution({
      ...
      contributionBuildId: Number(jobState.job.contributionBuild.id),
      note: outcomeSummary,
    });
...
async function retrySponsorCheckIn(operation) {
  let lastError;
  for (const delayMs of [0, 250, 750, 1_500]) {
    if (delayMs) await sleep(delayMs);
    try {
...
async function writeAssignment(jobState, state) {
  ...
  const relays = (jobState.relays || [])
    .map((relay) => {
      const details = [
        relay.summary,
        relay.requestedOutcome ? `What to deliver: ${relay.requestedOutcome}` : "",
        relay.constraints?.length
          ? `Constraints: ${relay.constraints.join("; ")}`
          : "",
        relay.acceptanceCriteria?.length
          ? `Done means: ${relay.acceptanceCriteria.join("; ")}`
          : "",
      ].filter(Boolean);
...
You are the same live ${displayProvider(state.operatorSession.provider)} agent session that opened sponsor duty. Zero or Ciel is the user's visible teammate; you are lending that teammate an added brain. Perform this work in this session. Do not launch a replacement coding provider or leave an unattended heartbeat process standing in for you.

The user approved sharing only this structured plan, active-job follow-ups, the assigned Build project, and its scoped Forum. Never inspect or infer from their private Zero/Ciel chat. Edit only contribution Build #${jobState.job.contributionBuild.id}; never merge Main, publish, or contact the user directly.

- User: @${jobState.job.requester.username}
- Visible teammate: ${displayPersona(jobState.job.persona)}
- Main project: ${jobState.job.rootBuild.title} (#${jobState.job.rootBuild.id})
- Assigned contribution branch: ${jobState.job.contributionBuild.title} (#${jobState.job.contributionBuild.id})
- Workspace: ${jobState.workspaceDir}

${relays || "No approved relay text was supplied."}

${jobState.forumContext ? `## Scoped Build Forum snapshot\n\n${jobState.forumContext}\n` : ""}
## Duty protocol

1. Run \`lumine sponsor job begin ${jobState.job.id}\` before editing.
2. Work only in the workspace above. Use your native same-session subagents only after registering each with \`helper-start\`. The coordinator alone runs sponsor CLI commands; helpers report their results back to it.
3. Run \`lumine sponsor job pulse ${jobState.job.id}\` between substantial work steps to keep the lease alive and receive approved follow-ups.
4. After applying a relay, record its exact ID with \`lumine sponsor job relay-applied ${jobState.job.id} <relay-id...>\`.
5. Finish with \`lumine sponsor job complete ${jobState.job.id} --summary "..."\`.
`;
...
function jobWorkspaceOptions(options, state, jobState, overrides = {}) {
  return {
    ...
    target: String(jobState.job.contributionBuild.id),
    buildIdFlag: String(jobState.job.contributionBuild.id),
...
async function archiveSponsorState(options) {
  const filePath = sponsorDutyStatePath(options);
