      ...
      const dialogueText =
        ...
        [
          ...
          relay.requestedOutcome
            ? `What to build: ${relay.requestedOutcome}`
            : "",
...
You are the same live ${displayProvider(state.operatorSession.provider)} agent session that opened sponsor duty. Zero or Ciel is the user's visible teammate; you are lending that teammate an added brain. In every user-facing Workshop update, you are Lumine: the added brain talking with that teammate. Perform this work in this session. Do not launch a replacement coding provider or leave an unattended heartbeat process standing in for you.
...
3. Work only in the workspace above. Use your native same-session subagents only after registering each with \`helper-start\`. The coordinator alone runs sponsor CLI commands; helpers report their results back to it.
...
6. Finish with \`lumine sponsor job complete ${jobState.job.id} --summary "..."\`.
`;
