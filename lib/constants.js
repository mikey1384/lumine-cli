...
// Mirrors the web workspace's runtime asset accept list. The server rejects
// video; client allowlists keep project asset uploads focused on files loaders
// can consume from the sandbox.
...
// Default platform project-file limits. A pulled workspace can carry larger
// canonical per-project limits approved on Main in lumine-project.json.
...
export const PROJECT_MAX_TOTAL_BYTES_DEFAULT = 1024 * 1024;
...
- \`lumine assets upload <file...>\` uploads supported runtime assets (images,
  ...
  survive pruning from project files. \`lumine assets list\`
  prints assets and refreshes
...
- CAUTION: asset URLs an app stores in privateDb/sharedDb/user DBs at runtime
  (Twinkle.files uploads), or URLs referenced only inside uploaded files such as
  .gltf manifests, are invisible to prune. Keep those asset URLs in project
  files too, or do not run \`assets prune\` for that build.
...
- Default max ${PROJECT_MAX_FILES} files and ${Math.floor(PROJECT_MAX_TOTAL_BYTES_DEFAULT / 1024)} KB per project. Read
  \`.twinkle/lumine-project.json\` for this project's canonical \`projectLimits\`;
  an approved Main project can allow up to 500 files and 5 MB, inherited by branches.
- Each file max ${PROJECT_MAX_EFFECTIVE_FILE_LINES} effective lines
