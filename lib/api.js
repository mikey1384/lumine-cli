...
export async function loadExternalAgentRuntime({
  ...
  return await requestJson({
    method: "POST",
    url: `${options.apiUrl}/cli/build/${buildId}/agent/runtime`,
    authToken: auth.token,
    body: {
      userMessage,
      projectFiles,
...
export async function executeExternalAgentTool({
  options,
  auth,
  buildId,
  baseFilesHash,
  ...
  return await requestJson({
    method: "POST",
    url: `${options.apiUrl}/cli/build/${buildId}/agent/tool`,
    authToken: auth.token,
    body: {
      baseFilesHash,
      projectFiles,
...
export async function validateExternalAgentCandidate({
  options,
  auth,
  buildId,
  baseFilesHash,
  ...
  return await requestJson({
    method: "POST",
    url: `${options.apiUrl}/cli/build/${buildId}/agent/validate`,
    authToken: auth.token,
    body: {
      baseFilesHash,
      userMessage,
...
export function createLumineSaveClientContext(options, environment = process.env) {
  ...
  const selectedExternalAgent =
    options?.externalAgentProvider === "claude-code"
      ? "claude_code"
      : options?.externalAgentProvider === "codex"
        ? "codex"
        : "";
  return {
    source: "lumine_cli",
    ...
    agentEnvironment:
      selectedExternalAgent || detectLumineAgentEnvironment(environment),
  };
}
