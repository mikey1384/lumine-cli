...
export async function runAutomaticPagination({
  ...
  if (
    options.adminOutput &&
    path.resolve(options.adminOutput) === checkpointPath
  ) {
    throw validationError(
  ...
  if (options.adminOutput) {
    result.data.scan.outputPath = path.resolve(options.adminOutput);
    await writePaginatedResultFile(options.adminOutput, result);
  }
  return result;
}
