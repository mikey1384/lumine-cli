...
async function sponsorDutyCommand(options, args, commandServices) {
  ...
  if (["pause", "resume", "stop"].includes(action)) {
    const auth = await ensureAuth(options);
    const status = await sponsorRequest({ options, auth, path: "/status" });
    const currentDuties = (status.duties || []).filter((duty) =>
    ...
    if (currentDuties.length > 1) {
      throw new Error(
        `Multiple legacy duty sessions are still open (${currentDuties.map((duty) => `#${duty.id}`).join(", ")}). Stop them with the CLI version that started them before using shared duty.`,
      );
    }
    const dutyId = Number(currentDuties[0].id || 0);
    if (!dutyId) throw new Error("Twinkle returned an invalid duty session.");
    const state =
    ...
    const result = await sponsorRequest({
      ...
      method: "POST",
      path: "/duty/state",
      body: { dutySessionId: dutyId, state },
    });
    printJsonOrLines(options, result, [`Sponsor duty is now ${state}.`]);
    return;
  }
...
async function runSponsorDuty({
  ...
  if (started.duty?.scope !== "shared") {
    throw new Error("Twinkle did not return a shared sponsor duty lease.");
  }
  ...
  const dutyHeartbeatEveryMs =
    Math.max(10, Number(started.heartbeatEverySeconds || 30)) * 1_000;
  let lastDutyHeartbeatSuccessAt = Date.now();
  const heartbeatDuty = async () => {
    ...
    try {
      ...
      dutyLeaseError = null;
      lastDutyHeartbeatSuccessAt = Date.now();
      ...
      // Same lease policy as the per-job heartbeat: end the duty only when the
      // server rejects the lease or the lease window has fully elapsed. A
      // transient network failure retries on the next tick.
      const terminalDutyError = !isRetryableSponsorDutyRequestError(error);
      const leaseWindowElapsed =
        Date.now() - lastDutyHeartbeatSuccessAt >=
        Math.max(120_000, dutyHeartbeatEveryMs * 3);
      if (terminalDutyError || leaseWindowElapsed) {
  ...
  const dutyHeartbeat = setInterval(
    () => void heartbeatDuty(),
    dutyHeartbeatEveryMs,
  );

  console.log(`Shared Zero/Ciel sponsor duty started with ${provider}.`);
  ...
  try {
    while (!stopRequested) {
      ...
      while (!stopRequested && activeJobs.size < concurrency) {
        ...
        try {
          ...
          if (!isRetryableSponsorDutyRequestError(error)) {
            dutyLeaseError = error;
            stopRequested = true;
            break;
          }
          // A failed claim must not kill the duty process; the heartbeat owns
          // lease validity, and the outer loop polls again after a sleep.
          console.error(
            `Workshop claim failed; retrying next poll: ${error?.message || error}`,
          );
          break;
        }
        if (!claim.job) break;
    ...
    if (!dutyLeaseError && canonicalDuty.state === "active") {
      const paused = await sponsorRequest({
        ...
        method: "POST",
        path: "/duty/state",
        body: { dutySessionId: dutyId, state: "paused" },
      });
  ...
  console.log("Shared Zero/Ciel sponsor duty stopped.");
}
...
async function processSponsorJob({
  ...
  const job = claim.job;
  const persona = normalizeJobPersona(job?.persona);
...
function isRetryableSponsorDutyRequestError(error) {
  const status = Number(error?.status || 0);
...
async function printSponsorStatus(options) {
  ...
  for (const duty of status.duties || []) {
    ...
    console.log(
      `Duty #${duty.id}: ${scope} · ${duty.state} · ${duty.provider} · model=${duty.requestedModel || "provider default"} · effort=${duty.requestedEffort || "provider default"}`,
    );
  }
}
