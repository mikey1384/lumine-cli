...
export function resolveGenerateModel(options) {
  const rawModel = String(options.model || "").trim();
  const model = GENERATE_MODEL_ALIASES[rawModel];
  if (!model) {
    throw new Error(
      "--model is required: gpt-image-2 (best quality, slower, pricier) or nano-banana (Gemini — faster, cheaper). No default model is applied.",
    );
  }
  ...
  if (rawQuality && model !== "gpt-image-2") {
    throw new Error(
      "--quality only applies to gpt-image-2; nano-banana has a single quality tier.",
    );
  }
  return { model, quality: rawQuality || null };
}

export function formatBatteryPercent(energyUnits, fullBatteryUnits) {
  const total = Number(fullBatteryUnits);
  const units = Number(energyUnits);
  if (
...
export function describeGenerateHttpError(error) {
  const code = String(error?.data?.code || "").trim();
  if (error?.status === 429 && code === "ai_energy_depleted") {
    return "Your AI battery is empty. Recharge on Twinkle or try again tomorrow.";
  }
  if (error?.status === 409) {
    return "Another image generation is already running for your account — wait for it to finish, then retry.";
  }
  if (code === "build_runtime_file_storage_limit_exceeded") {
    return `${error?.data?.error || "Storage quota exceeded."} Free space with \`lumine assets prune\`.`;
  }
  if (error?.status === 404) {
    return "Twinkle API does not support asset generation yet (deploy the latest API first).";
  }
  return null;
}

export async function assetsGenerate(options) {
  ...
  const fileName = String(options.assetName || "").trim();
  ...
  if (!options.assumeYes) {
    let estimate = null;
    try {
      const estimateResult = await buildApiJson({
        options,
        auth,
        buildId,
        buildApiToken: token,
        endpointPath: "api/assets/generate",
    ...
    const costLine = selectedOption
      ...
      : "  Estimated battery cost: unavailable";
    const remainingLine = estimate
      ...
      : "";
    console.log(
      `Generate 1 image with ${model}${quality ? ` (quality ${quality})` : ""}?`,
    );
  ...
  console.log("Generating image... this can take a minute.");
  let result;
  try {
    result = await buildApiJson({
      options,
      auth,
      buildId,
      buildApiToken: token,
  ...
  if (!asset?.url) {
    throw new Error("Generation finished but no asset was returned.");
  }
  console.log(
    `Generated ${asset.originalFileName || asset.fileName} (${formatBytes(asset.sizeBytes)}) as asset #${asset.id}`,
  );
  console.log(`  ${asset.url}`);
  const remaining = result?.aiUsagePolicy?.energyRemaining;
  if (Number.isFinite(Number(remaining))) {
    console.log(
      `Battery remaining: ${formatBatteryPercent(Number(remaining), result?.aiUsagePolicy?.baseEnergyUnitsPerDay)}`,
    );
  }
  console.log(
    "Reference this URL from your project files (e.g. a src/assets.js module).",
  );
...
export async function assetsUpload(options) {
  ...
  const referencedGltfAssetUrls = candidates.flatMap(
    (candidate) => candidate.referencedGltfAssetUrls || [],
  );
  let existingAssets = [];
  let existingProjectAssets = [];
  try {
    const listedAssets = await fetchAllRuntimeAssets({
      options,
      auth,
      buildId,
      buildApiToken: token,
    });
    existingAssets = listedAssets.assets;
    existingProjectAssets = listedAssets.projectAssets || [];
  } catch (error) {
    if (referencedGltfAssetUrls.length > 0) {
      throw new Error(
        "Could not verify this glTF's Twinkle asset URLs. Refresh the asset list and try again.",
        { cause: error },
      );
    }
  }
  if (referencedGltfAssetUrls.length > 0) {
    const knownAssets = [...existingAssets, ...existingProjectAssets];
    const unverifiedUrls = referencedGltfAssetUrls.filter(
      (referencedUrl) =>
        !knownAssets.some((asset) =>
          runtimeAssetUrlMatchesGltfReference(asset, referencedUrl),
        ),
    );
    if (unverifiedUrls.length > 0) {
      throw new Error(
        `The glTF references Twinkle asset URL${
          unverifiedUrls.length === 1 ? "" : "s"
        } that do not belong to this Build: ${unverifiedUrls
          .slice(0, 3)
          .map((url) => `"${url}"`)
          .join(
            ", ",
          )}${unverifiedUrls.length > 3 ? ", ..." : ""}. Upload or reuse assets listed by \`lumine assets list\`, then use their exact URLs.`,
      );
    }
  }
  if (existingAssets.length > 0 || existingProjectAssets.length > 0) {
    try {
      ...
      // Duplicate-name warnings are advisory only.
    }
  }
  const uploaded = [];
  for (const candidate of candidates) {
...
// snapshot check cannot be done client-side. Structural blind spots, always
// disclosed: references stored in app data (privateDb/sharedDb/viewer DB by
// Twinkle.files-using apps), and references stored only inside uploaded asset
// files such as .gltf manifests, are invisible to every scan.
export async function assetsPrune(options) {
  ...
  console.log(
    "Caution: asset URLs stored in app data (privateDb/sharedDb/user DBs by apps using Twinkle.files), or referenced only inside uploaded files such as .gltf manifests, cannot be detected. Keep those URLs in project files too, or do not prune this build's assets.",
  );
  if (!serverVerified) {
    console.log(
  ...
  for (const asset of unreferenced) {
    ...
    console.log(
      `Deleted #${asset.id} ${asset.originalFileName || asset.fileName}`,
    );
  }
...
export async function putAssetPartWithRetry({
  url,
  chunk,
  mimeType,
  partLabel,
...
export async function readAssetUploadCandidate(filePath) {
  ...
  if (!mimeType) {
    throw new Error(
      `Unsupported asset type for ${fileName}. Build assets support runtime media files (${Object.keys(ASSET_MIME_BY_EXTENSION).join(", ")}).`,
    );
  }
  let buffer;
  try {
  ...
  const referencedGltfAssetUrls =
    extension === ".gltf"
      ? assertGltfUploadCandidateHasOnlySupportedUris({ fileName, buffer })
      : [];
  return {
    absolutePath,
    fileName,
    mimeType,
    buffer,
    referencedGltfAssetUrls,
  };
}

const TWINKLE_GLTF_ASSET_HOSTS = new Set([
  "d3jvoamd2k4p0s.cloudfront.net",
  "www.twin-kle.com",
  "twin-kle.com",
]);
const TWINKLE_GLTF_ASSET_PATH_PREFIXES = [
  "/attachments/build-runtime/",
  "/attachments/optimized/",
];

function isSupportedGltfUri(uri) {
  const normalizedUri = String(uri || "").trim();
  if (!normalizedUri) return true;
  const schemeMatch = normalizedUri.match(/^([A-Za-z][A-Za-z0-9+.-]*):/);
  if (!schemeMatch) return false;
  if (schemeMatch[1].toLowerCase() === "data") return true;
  let parsedUri;
  try {
  ...
  return (
    ...
    !parsedUri.search &&
    !parsedUri.hash &&
    TWINKLE_GLTF_ASSET_HOSTS.has(parsedUri.hostname.toLowerCase()) &&
    TWINKLE_GLTF_ASSET_PATH_PREFIXES.some((prefix) =>
      parsedUri.pathname.startsWith(prefix),
    )
  );
}

function collectUnsupportedGltfUris(parsedGltf) {
  const unsupportedUris = [];
  if (!parsedGltf || typeof parsedGltf !== "object") {
    return unsupportedUris;
  }
  for (const entry of [
    ...(Array.isArray(parsedGltf.buffers) ? parsedGltf.buffers : []),
    ...(Array.isArray(parsedGltf.images) ? parsedGltf.images : []),
  ]) {
    const uri = typeof entry?.uri === "string" ? entry.uri.trim() : "";
    if (uri && !isSupportedGltfUri(uri)) {
      unsupportedUris.push(uri);
    }
  }
  return unsupportedUris;
}

function assertGltfUploadCandidateHasOnlySupportedUris({ fileName, buffer }) {
  let parsedGltf;
  try {
    parsedGltf = JSON.parse(buffer.toString("utf8"));
  } catch {
    throw new Error(`Invalid .gltf JSON in ${fileName}.`);
  }
  const unsupportedUris = collectUnsupportedGltfUris(parsedGltf);
  ...
  const sampleUris = unsupportedUris
    .slice(0, 3)
    .map((uri) => `"${uri}"`)
    .join(", ");
  throw new Error(
    `${fileName} contains unsupported glTF URI${
      unsupportedUris.length === 1 ? "" : "s"
    } (${sampleUris}${unsupportedUris.length > 3 ? ", ..." : ""}). Raw .gltf companion files are stored under separate asset URLs, and Build apps cannot load arbitrary external URLs. Use .glb, embed data URIs, or rewrite .gltf URIs to Twinkle runtime attachment URLs before uploading.`,
  );
}

function runtimeAssetUrlMatchesGltfReference(asset, referencedUrl) {
  const normalizedReference = String(referencedUrl || "").trim();
  if (!normalizedReference) return false;
  return [asset?.url, asset?.thumbUrl].some(
    (assetUrl) => String(assetUrl || "").trim() === normalizedReference,
  );
}
...
export async function uploadRuntimeAsset({
  ...
  try {
    ...
    for (let partNumber = 0; partNumber < urls.length; partNumber += 1) {
      ...
      const end = Math.min(
        start + ASSET_UPLOAD_CHUNK_BYTES,
    ...
    if (!asset?.url) {
      throw new Error(
        `Upload of ${candidate.fileName} completed without an asset URL.`,
      );
    }
    return asset;
  } catch (error) {
...
export async function writeAssetsManifest({
  ...
  const unmatchedReferences = refs
    ? refs.filter(
        (ref) =>
          !knownAssets.some((asset) =>
            runtimeAssetMatchesReference(asset, ref),
          ),
      )
    : null;
    ...
    JSON.stringify(
      {
        ...
        ...(annotatedProjectAssets
          ? { projectAssets: annotatedProjectAssets }
          : {}),
