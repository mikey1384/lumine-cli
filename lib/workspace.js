...
// Local mirror of the canonical per-project limits returned by the server so
// violations fail fast with the full list (the server stops at the first).
export function collectProjectLimitFindings(files, projectLimits = {}) {
  const errors = [];
  const warnings = [];
  const canonicalMaxFiles = Math.floor(
    Number(projectLimits?.maxFilesPerProject || 0),
  );
  const canonicalMaxProjectBytes = Math.floor(
    Number(projectLimits?.maxProjectBytes || 0),
  );
  const maxFilesPerProject =
    canonicalMaxFiles > 0 ? canonicalMaxFiles : PROJECT_MAX_FILES;
  const maxProjectBytes =
    canonicalMaxProjectBytes > 0
      ? canonicalMaxProjectBytes
      : PROJECT_MAX_TOTAL_BYTES_DEFAULT;
  const warnProjectBytes = Math.min(
    Math.floor(maxProjectBytes * 0.8),
    maxProjectBytes - 1,
  );
  if (files.length > maxFilesPerProject) {
    errors.push(
      `Project exceeds the ${maxFilesPerProject}-file limit (${files.length} files). Split or consolidate files before saving.`,
    );
  }
  for (const file of files) {
  ...
  if (totalBytes > maxProjectBytes) {
    errors.push(
      `Project files total ${formatBytes(totalBytes)}, above this project's ${formatBytes(maxProjectBytes)} limit.`,
    );
  } else if (totalBytes > warnProjectBytes) {
    warnings.push(
      `Project files total ${formatBytes(totalBytes)} — approaching this project's ${formatBytes(maxProjectBytes)} cap. Keep files lean where structure allows.`,
    );
  }
  return { errors, warnings };
}

export function assertProjectFilesWithinLimits(files, projectLimits = {}) {
          ...
          ? {
              ...
              canPublish:
                build.canPublish !== undefined
                  ...
                  : build.role === "owner",
              projectLimits:
                build.projectLimits && typeof build.projectLimits === "object"
                  ? build.projectLimits
                  : null,
            }
          : null,
