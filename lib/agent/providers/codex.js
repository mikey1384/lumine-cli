...
async function runCodexAppServerTurn({
  ...
  try {
    ...
    const inheritedToolServers = (Array.isArray(mcpInventory?.data)
      ? mcpInventory.data
      : []
    )
...
async function loadCodexIsolationArgs({ binary, cwd, environment }) {
...
async function runCodexInspectionCommand({
  ...
  return await new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      child.kill();
      finish(
        new Error(`Codex ${label} timed out; Lumine cannot prove tool isolation.`),
      );
