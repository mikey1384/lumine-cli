...
export function parseAdminOperation(options) {
  ...
  if (namespace === "runtime") {
    if (action !== "evidence" || (target && !["primary", "target"].includes(target)) || extra) {
      throw cliValidationError("Usage: lumine admin runtime evidence [primary|target] [--days 1-7] [--json].");
    }
    const days = Number(options.adminDays || 7);
    if (!Number.isInteger(days) || days < 1 || days > 7) {
      throw cliValidationError("--days must be an integer between 1 and 7.");
    }
    return readOperation("runtime.evidence", withQuery(`/cli/admin/runtime-logs/hosts/${target || "primary"}/evidence`, { days }), { requiresRun: false });
  }
