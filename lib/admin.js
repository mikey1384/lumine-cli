...
export async function adminCommand(options) {
  ...
  try {
    ...
    if (operation.name === "builds.candidates") {
      result = normalizeAdminBuildCandidatesResult({
        result,
...
export function parseAdminOperation(options) {
  ...
  if (namespace === "comment") {
    if (action === "draft" || action === "reply") {
      ...
      if (parsedTarget.type === "build") {
        ...
        if (!reviewedBuildVersionId || !buildReviewMethod) {
          throw cliValidationError(
            "After reviewing the project, pass both --reviewed-version <published-artifact-id> and --reviewed-via runtime|code.",
          );
        }
      } else if (
  ...
  throw cliValidationError(
    "Usage: lumine admin identity|daily-run|recommendations|builds|post|subjects|subject|featured|comment|chat|news|audit|brief|bot-output|notable ...",
  );
}
