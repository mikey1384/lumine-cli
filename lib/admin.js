...
export function readRewardConfigFile(filePath) {
  ...
  if (!Array.isArray(parsed.rules) || parsed.rules.length === 0) {
    throw cliValidationError(
      "Approval needs at least one earning rule in rules[] (id, title, xp, coins, verifier: \"numeric-quiz\", questions[{prompt, answer}]).",
    );
  }
  return parsed;
}
...
export async function adminCommand(options) {
  ...
  try {
    ...
    if (operation.featuredWorkflow) {
      result = await runFeaturedWorkflow({ options, operation, authToken: auth.token, runId });
    ...
    if (operation.featuredWorkflow && error.featuredProgress) {
      ...
      error.data = {
        ...
        error: {
          code: serverError?.code || error.code || "LUMINE_ADMIN_FEATURED_WORKFLOW_FAILED",
          message: error.message,
          details: {
            ...(serverError && typeof serverError === "object" ? serverError.details : {}),
            ...error.featuredProgress,
            retryInstruction: "Resume the exact command with --resume; confirmed items are not replayed.",
          },
        },
      };
    }
...
async function finishAdminOutput({ options, operation, result }) {
  ...
  if (
    ...
    writeAdminResultOutput({ filePath: options.adminOutput, result, operation });
  }
  if (options.json) {
...
export function assertAdminOperationAllowedForRunScope({
  ...
  if (runScope === "newspaper") {
    if (
      ["news.status", "news.claim", "news.submit", "news.print", "daily-run.complete", "daily-run.fail"].includes(operation.name)
    ) return;
    throw cliValidationError(
    ...
    if (REWARD_REVIEW_DECISIONS.includes(action)) {
      ...
      if (action === "approve") {
        if (!options.adminConfigFile) {
          throw cliValidationError(
            "Approval needs the reviewer's earning rules: pass --config <rules.json>. Read the request with `reward-review show <id> --dir <path>` first.",
          );
        }
  ...
  if (namespace === "featured") {
    if (action === "comments") {
      if (!["scan", "acknowledge", "recommend", "report"].includes(target)) {
        throw cliValidationError("Usage: featured comments scan|acknowledge|recommend|report --checkpoint <file>.");
      }
      if (options.adminAll || options.adminCursor || options.adminUnviewed || options.adminViewed || options.adminAfter) {
        throw cliValidationError("The Featured review covers every comment; filtered or caller-supplied cursors are not supported.");
      }
      return { ...writeOperation(`featured.comments.${target}`, "POST", "/cli/admin/subjects/featured/reviews", {}),
        mutates: target !== "report", featuredWorkflow: target };
    }
    if (action === "plan") {
      if (!options.adminPostedAfter) throw cliValidationError("A Featured plan requires --posted-after <timestamp>.");
      return writeOperation("featured.plan", "POST", "/cli/admin/subjects/featured/plan", {
        removeIds: parseFeaturedSubjectIds(options.adminRemoveIds, "--remove-subject-ids"),
        addIds: parseFeaturedSubjectIds(options.adminAddIds, "--add-subject-ids"),
        ...(options.adminIds ? { finalIds: parseOrderedIds(options.adminIds) } : {}),
        postedAfter: options.adminPostedAfter,
      });
    }
    if (action === "apply") {
      readApprovedFeaturedPlan(options.adminFile, options.adminApprove);
      return { ...writeOperation("featured.apply", "POST", "/cli/admin/subjects/featured/plan/apply", {}),
...
function featuredRotateOperation(options) {
  ...
  return writeOperation(
    ...
    "/cli/admin/subjects/featured/rotation",
    {
      removeIds,
      addIds,
      ...(options.adminPostedAfter ? { postedAfter: options.adminPostedAfter } : {}),
    },
  );
}
...
function printAdminEnergyBudget(energyBudget) {
  ...
  if (headline) {
    ...
    console.log("AI Energy budget health: no completed UTC day in the requested window.");
  }
  console.log(
  ...
  console.log(`Flags (${flags.length}) — record each as a carry-over todo and escalate; never auto-enforce:`);
...
function printRewardReviewResult({ operation, data }) {
  ...
  if (Array.isArray(config.rules)) {
    console.log(`Budgets: day ${config.dailyXP} XP/${config.dailyCoins} Coins · per user/day ${config.userDailyXP} XP/${config.userDailyCoins} Coins · lifetime ${config.lifetimeXP} XP/${config.lifetimeCoins} Coins`);
    for (const rule of config.rules) {
      console.log(`  rule ${rule.id}: ${rule.title} · ${rule.xp} XP + ${rule.coins} Coins · ${Array.isArray(rule.questions) ? rule.questions.length : 0} question(s)`);
    }
  }
...
function printAdminResult({ operation, result }) {
  ...
  if (operation.name === "featured.plan") {
    ...
    console.log(`Proposed final order: ${(data.plan?.finalIds || []).join(",")}`);
    console.log(`Plan hash: ${data.planHash}. Obtain Mikey's approval before using featured apply --file <plan.json> --approve ${data.planHash}.`);
    return;
  }
  if (operation.featuredWorkflow) {
