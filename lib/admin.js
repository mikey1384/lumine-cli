...
function finishAdminOutput({ options, operation, result }) {
  if (options.json) {
...
function printAdminMonthlyMediaCosts(monthlyMediaCosts) {
  ...
  console.log(
    `Operations: clips ${operations.clips.completingCount} completing / ${operations.clips.processingCount} processing / ${operations.clips.staleCount} stale; live ${operations.live.liveCount} broadcasting / ${operations.live.costBearingChannelCount} cost-bearing channel(s) / ${operations.live.cleanupOverdueCount} cleanup overdue; viewers ${operations.viewers.activeGrantCount} active / ${operations.viewers.expiredActiveGrantCount} expired-active.`,
  );
  console.log(
  ...
  console.log(
    "These are conservative provider-cost ledger estimates, not an AWS invoice; reconcile IVS, MediaConvert, and shared S3 Cost Explorer data separately after billing lag.",
  );
}
